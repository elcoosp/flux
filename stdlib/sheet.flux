// sheet.flux — `Sheet` adapter compo (FLUX-038).
//
// Bottom-anchored sheet that slides up over the current scene. Overlay
// container: `content` is its children. The presentation is data the host
// consumes (iOS `.sheet` / Compose `ModalBottomSheet`); never a wire animation
// frame (ADR-0047 + FLUX-038).
//
