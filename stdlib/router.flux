...
// `Router` declares an `initial` route prop (the host still falls back to signal
// 97 / the first Screen) and `Screen` takes its `route` as a named prop — the
// exact prop the iOS / Android reconcilers read via `FNV-1a("route")` to pick
// the visible screen (ADR-0045).
//
...
compo Router(initial: String = "home")
