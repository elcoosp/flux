---
id: FLUX-077
status: done      # All 9 FLUX-077 primitives (Stack/Grid/Spacer/SafeArea/Modal/Sheet/Dialog/Animate/Toggle) have prop-contract parity with Android: degraded adapters RECORD the same data props (flex/edges/columns/gap/onDismiss/signal/curve/duration) the Android LayoutOverlayAdapterTest pins, and the iOS LayoutOverlayAdapterTests + the runtime RenderMountTests.testRegistryResolvesAllFlux077Primitives assert them. Native PRESENTATION stays gated on ADR-0048 by design (degraded container = correct unified-tier mapping, AGENTS.md §0.2). Filer intent (degraded-acceptable vs real parity) settled 2026-08-30: degraded form accepted as parity.
#
# PROOF (2026-08-30, iPhone 17 Pro simulator):
# - adapters/ui-swift SwiftPM package: 73 tests, 0 failures (xcodebuild -scheme FluxUIKit test).
# - runtimes/ios FluxAppTests/RenderMountTests: 7 tests, 0 failures (xcodebuild -scheme FluxApp test) incl. testRegistryResolvesAllFlux077Primitives.
# The earlier non-compiling FluxHost/FluxBytecodeVM non-exhaustive-switch blocker was
# resolved by a concurrent lane; the RenderMountTests target now builds + passes.
lane: LANE-N
phase: "Phase 2"
blocked_by:
  - FLUX-037
