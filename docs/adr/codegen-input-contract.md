# ADR codegen-input-contract: codegen takes `(LoweredIr, Ast)`, not `LoweredIr` alone

- **Status:** Accepted (created 2026-08-25 by the orchestrator)
- **Supersedes the recommendation in:** `docs/adr/codegen-kotlin-loweredir-name-gap.md`
...
- **Related:** FLUX-020 (codegen-swift, DONE), FLUX-021 (codegen-kotlin, PENDING),
  ADR-0027 (node-ID bridge), `docs/agents-boundaries-contract.md` Batch 5
...
`docs/adr/codegen-kotlin-loweredir-name-gap.md`, which concluded FLUX-020/021 were
...
- **Gap G3 independence:** closing Gap G3 (`arena.string_table()` populated, see
  `docs/adr/flux018-string-table-gap.md`) is needed by the **dev server / wire codec
...
3. **Gap G3 (component/primitive names in `LoweredIr`, and the arena string table)
   ...
   codegen. It is tracked separately (`flux018-string-table-gap.md`, now closed in
