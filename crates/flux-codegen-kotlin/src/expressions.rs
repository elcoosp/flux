...
/// standalone Kotlin statement; this is deliberately distinct from
...
/// renders it as a sequence of Kotlin statements.
///
/// Returns `None` when the handler is absent or empty, so callers can emit a
/// bare `{ }` lambda (a button with no behaviour still compiles and parses).
#[must_use]
