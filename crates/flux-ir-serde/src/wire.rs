...
use flux_syntax::{
    Child, ClosureRef, FileId, HandlerId, NodeKind, NodeRef, Patch, PropDiff, Props, SignalId,
    Span, StringId, Value,
};
...
/// Encodes a handler definition (Appendix D §D.8). Reserved for the dev server
/// frame layer, which ships handler closures in a dedicated section.
#[allow(dead_code)]
...
/// Decodes a handler definition (Appendix D §D.8).
#[allow(dead_code)]
