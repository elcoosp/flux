        ...
        loop {
            tokio::select! {
                _ = tokio::time::sleep(Duration::from_millis(250)) => {
                    ...
                    let signal_val = Value::Int(count as i64);
                    let regs: [Value; 16] = std::array::from_fn(|i| {
                        if i == 0 { signal_val.clone() } else { Value::Null }
                    });
                    let events = vec![
                    ...
                    if w.send(Message::Binary(enriched_frame(
                        events.into_iter().map(flux_ir_serde::enrich_telemetry).collect(),
                    )
                    .into()))
                    .await
                    .is_err()
