...
pub(crate) fn emit_primitive(em: &mut Emitter<'_>, id: NodeId, indent: usize) {
    ...
    match name.as_str() {
        ...
        "Button" => {
            ...
            em.line(indent, &format!("Button(action: {{ {handler} }}) {{"));
            em.line(indent + 4, &format!("Text({label})"));
            em.line(indent, "}");
        }
        "Image" => {
            let value = primary
...
/// Finds the `onClick`/`onTap` handler argument and renders its body as Swift
/// statements. Returns an empty string when no handler is present, so the
/// closure is still valid (`Button(action: { }) { … }`).
