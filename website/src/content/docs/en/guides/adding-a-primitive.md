...
(see Appendix F). Adding one is a cross-cutting change: the adapter contract in
the spec, the dev implementation, the release implementation, and a parity test
...
- **Dev (iOS/Android):** drive `UISlider` / `android.widget.SeekBar` (or
  Compose `Slider`) imperatively in the adapter's `update`.
...
If the primitive writes a signal (here `onValueChange` fires on drag), the host
must register the handler and the node's `signal_deps` must include whatever the
handler closure reads. This is what keeps the dirty-set reconcile correct.

## 4. Add a parity test

Add a golden scenario to `reconcile-trace-format.md` (e.g. `slider_drag`) and a
trace in `/tests/trace-goldens/`. The parity tool (`flux-parity trace diff`)
proves the Swift and Kotlin hosts produce byte-identical traces.
