...
/**
 * LANE-B device-only blind spot: a POSITIONAL `Screen("home")` lowers its route
 ...
 * NAMED `Screen(route:)` form (see the G7 tests above).
 */
@Test
fun `positional screen route at index 0 never swaps on navigate`() =
    runTest {
        val dispatcher = StandardTestDispatcher(testScheduler)
        val scope = TestScope(dispatcher)
        val signals = SignalGraph()
        ...
        val bytes =
            FrameBuilder()
                .apply {
                    ...
                    stringEntry(settingsId, "settings")
                    node(id = 1u, kind = 0x12u, component = 600u, props = emptyList(), childIds = listOf(2u, 3u))
                    // POSITIONAL form: route at prop index 0, not FNV-1a("route").
                    node(id = 2u, kind = 0x12u, component = 500u, props = listOf(0u.toUShort() to WireValue.StrVal(homeId)), childIds = emptyList())
                    node(id = 3u, kind = 0x12u, component = 500u, props = listOf(0u.toUShort() to WireValue.StrVal(settingsId)), childIds = emptyList())
                }.build()
        ...
        // A real tap of "Go to Settings": LOAD_STR_CONST r0, settingsId ; CALL_CAP r1, (3,1), args=r0 ; HALT.
        val navigateBytecode =
            byteArrayOf(
                0xB3.toByte(), // LOAD_STR_CONST
                0, // result reg r0
                (settingsId and 0xFFu).toByte(),
                0,
                0,
