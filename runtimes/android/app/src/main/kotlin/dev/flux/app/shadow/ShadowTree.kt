...
public class ShadowTree(
    ...
    private fun signalDepsFrom(props: List<Pair<UShort, dev.flux.app.wire.WireValue>>): MutableSet<UInt> {
        val set = LinkedHashSet<UInt>()
        for ((_, value) in props) {
            if (value is dev.flux.app.wire.WireValue.IntVal) set.add(value.value.toUInt())
        }
    ...
    private fun signalDepsFromWireInto(
        props: List<Pair<UShort, dev.flux.app.wire.WireValue>>,
        set: MutableSet<UInt>,
    ) {
        for ((_, value) in props) {
            when (value) {
                is dev.flux.app.wire.WireValue.IntVal -> set.add(value.value.toUInt())
                is dev.flux.app.wire.WireValue.ListVal -> for (item in value.items) collectInts(item, set)
                is dev.flux.app.wire.WireValue.RecordVal -> for (f in value.fields) collectInts(f.value, set)
                else -> Unit
            }
        }
    }
    ...
    private fun collectInts(
        value: dev.flux.app.wire.WireValue,
        set: MutableSet<UInt>,
    ) {
        when (value) {
            is dev.flux.app.wire.WireValue.IntVal -> set.add(value.value.toUInt())
            is dev.flux.app.wire.WireValue.ListVal -> value.items.forEach { collectInts(it, set) }
            is dev.flux.app.wire.WireValue.RecordVal -> value.fields.forEach { collectInts(it.value, set) }
            else -> Unit
        }
    }

    /** Materializes a kit [Props] from raw wire values. */
    internal fun kitFromWire(fields: List<Pair<UShort, dev.flux.app.wire.WireValue>>): Props =
        Props(
            fields.map {
                dev.flux.ui.Props
