...
//  fault captured by `FluxExecutor` (no VM error is ever allowed to escape).
...
struct FluxRootView: View {
    ...
    init() {
        // A minimal registry so the reconciler has adapters to consult; the
        // dev host seeds an empty graph until the first Init frame arrives.
        let registry = AdapterRegistry([MockAdapter(handles: .component)])
        _executor = State(initialValue: FluxExecutor(graph: SignalGraph(), registry: registry))
    }
