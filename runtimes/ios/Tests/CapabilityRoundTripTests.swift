...
final class CapabilityRoundTripTests: XCTestCase {
    /// The app shell (`FluxAppMain`, the `@main` entry) injects the real-OS
    /// `IOSNativeCapabilityHost` into `CapabilityRegistry.realNativeHost` at launch.
    /// These tests assert the *dev* host's deterministic echo contract, so we clear
    /// the injected host before each test (which also invalidates the cached `.dev`
    /// singleton) to keep `.dev` on the dev host.
    override func setUp() {
        super.setUp()
        CapabilityRegistry.realNativeHost = nil
    }
    ...
    // MARK: - FLUX-047: Http (cap 14) + Persist (cap 15) host bodies

    func testPersistPutThenGetRoundTrips() throws {
        var signals: any SignalStore = InMemorySignals()
        let key: FluxHost.FluxValue = .str(7)
        let value: FluxHost.FluxValue = .list([.int(1), .int(2)])
        let putArgs: FluxHost.FluxValue = .record([(UInt16(0), key), (UInt16(1), value)])
        let putCell = try CapabilityRegistry.dev.lookup(15, 1)!(15, 1, putArgs, &signals)
        guard case .list = signals.read(putCell) else {
            XCTFail("Persist.put returns the stored value")
            return
        }

        let getArgs: FluxHost.FluxValue = .record([(UInt16(0), key)])
    ...
    func testPersistQueryEnumeratesEntries() throws {
        ...
        var signals: any SignalStore = InMemorySignals()
        _ = try registry.lookup(15, 1)!(15, 1, .record([(0, .str(100)), (1, .int(11))]), &signals)
        _ = try registry.lookup(15, 1)!(15, 1, .record([(0, .str(200)), (1, .int(22))]), &signals)
    ...
    func testPersistDeleteClearsValue() throws {
        var signals: any SignalStore = InMemorySignals()
        let key: FluxHost.FluxValue = .str(7)
        _ = try CapabilityRegistry.dev.lookup(15, 1)!(15, 1, .record([(UInt16(0), key), (UInt16(1), .int(99))]), &signals)
        _ = try CapabilityRegistry.dev.lookup(15, 4)!(15, 4, .record([(UInt16(0), key)]), &signals)
        let getCell = try CapabilityRegistry.dev.lookup(15, 2)!(15, 2, .record([(UInt16(0), key)]), &signals)
    ...
    func testHttpGetJsonResolvesToRecordViaResolver() async throws {
        ...
        guard case .record = settled else {
            XCTFail("Http.getJson response parses to a record")
            return
        }
    }
